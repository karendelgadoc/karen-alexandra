...
export default function EditFashionNewsPage({ params }: Props) {
  ...
  useEffect(() => {
    params.then(({ id: postId }) => {
      setId(postId);
      fetch(`/api/admin/fashion-news/${postId}`)
        .then((r) => r.json())
        .then((post) =>
          setForm({
            slug: post.slug,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    setError(null);
    const res = await fetch(`/api/admin/fashion-news/${id}`, {
      method: "PUT",
  ...
  return (
    <div>
      <h1 className="text-2xl font-light mb-8">Edit Fashion News Article</h1>
